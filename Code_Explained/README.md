...
*   **Main Operation Loop:**
    *   The loop constantly checks a specific order of events:
        ...
        2.  **Continuous Ultrasonic Monitoring:** The ultrasonic sensors are the least important part of the code. If a wall is detected, the robot turns mildly away from it, and then continues forward. This is done to avoid walls in case Cap. gets too close to them. This is vital to ensure the HuskyLens has a clear view of all the obstacles. If it's too close to one side, it's possible he’ll miss the detection of certain obstacles.
        3.  **Color Detection & Reaction:** The robot simultaneously looks for colors. He has two given instructions in case he detects one of the two possible sets of colors:
            *   **Blue/Orange:** The robot will continue a forward motion for a few seconds longer, sort of a delay before turning. This is so that he has advanced enough to turn and bump into the wall behind him. He first turns in the corresponding direction, then reverses to bump into the wall. This way, he can always center himself and is awa.re of all the objects in front of him. This prevented him from turning too sharply or too widely and missing obstacles around him. When completing this sequence, he also tallies it up so as to keep track of how many laps are missing. Once he reaches 12, he will stop. Lastly, after the detection of one of the colored lines, he enters a pause period where it ignores further blue/orange lines so as to not detect the same line when going forward again.

            *   **Red/Green:** This feature allows him to avoid obstacles. When detecting either green or red, he stops, reverses briefly so as to not bump into the obstacle, and then performs a sequenced turn (left-then-right or right-then-left) to navigate around the virtual obstacle before continuing forward. When he makes this maneuver he ensures the obstacle is avoided, but also places himself back in his path.

*   Once the robot completes its maximum number of sequences (12), it stops its motor, bringing the course run to a complete end.
